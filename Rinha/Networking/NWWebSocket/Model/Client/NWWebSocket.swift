...
open class NWWebSocket: WebSocketConnection {
    ...
    public convenience init(name: String,
        ...
        if isSecure {
            url = URL(string: "wss://\(name).local:\(port)")!
        } else {
            url = URL(string: "ws://\(name).local:\(port)")!
        }
        
        self.init(url: url, connectAutomatically: connectAutomatically, options: options, connectionQueue: connectionQueue)
    }
